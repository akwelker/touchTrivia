...
Vector<char> answers =  Vector<char>();
int turn;
int score;
char ans = 'x';
...
void setup()
{
  ...
  questions.push_back("What is the Captial of Brazil?\n a.)Manaus\t b.) Sao Paulo\n c.)Rio de Janeiro\t d.)Brasilia");
  answers.push_back("d");
  
  questions.push_back("What is the Captial of Australia?\n a.)Camberra\t b.) Sidney\n c.)Perth\t d.)Adelaide");
  answers.push_back("a");
  
  questions.push_back("What is the Captial of Slovakia?\n a.)Zilina\t b.)Presov\n c.)Bratislava\t d.)Kosice");
  answers.push_back("c");
  
  questions.push_back("What is the Captial of Belgium?\n a.)Antwerp\t b.)Ghent\n c.)Brussels\t d.)Bruges");
  answers.push_back("c");
...
void loop()
{
  ...
  else if(turn == answers.size()) // or is ending 
  {
    Serial.print("Thanks for playing your score was: ");
    Serial.print(score);
    Serial.print('/');
    Serial.println(answers.size());
    turn++;
  }
  else if(turn % 3 == 0) // phase 1
  {
    Serial.println(questions.at(turn/3)); // print the question, move on!
    turn++;
  }
  ...
  else if(turn % 3 == 2) // phase 3
  {
    ...
    bool correct = (ans == answers.at(turn/3));
    ...
    else
    {
      ...
      Serial.println(answers.at(turn/3));
    }
